		...
		<div class="row">
			<nav class="main-nav">
				...
				<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
				    <h2 class="lvl-name"><?php echo $level_title->STG_DESCRIPTION; ?>-<?php echo $level_title->LVL_NUM; ?></h2>
				</div>
						    ...
						    <section>
								<div class="tutorialDescription row">
								    ...
								    <?php foreach ($goal_list as $g) { ?>
								    	...
								    	<div style="" id="qstn_tag_<?php echo $g['G_ID'] ?>" class="question-tag-container col-sm-4"></div>
								    	<br>
								    <?php } ?>
								</div>
						    </section>
					...
					<div class="modal-body">
						<div class="objectives" id="objectives">
							...
							<?php foreach ($goal_list as $g) { ?>
						    	...
						    	<div style="" id="finish_qstn_tag_<?php echo $g['G_ID'] ?>" class="question-tag-container col-sm-4"></div>
						    	<br>
						    <?php } ?>
	...
	<div class="modal-content">
		...
		<div class="objectives container" style="margin-top: 30;">
			<?php foreach ($goal_list as $g) { ?>
		    	...
		    	<div style="" id="lose_qstn_tag_<?php echo $g['G_ID'] ?>" class="question-tag-container col-sm-4"></div>
		    	<br>
		    <?php } ?>
	...
	$(document).ready(function() {
		...
		preloadGameData = function() {
			...
			var promise = new Promise(function(resolve, reject) {
				...
				for(var key in gameData.question_list.question_list) {

					gameData.question_list.question_list[key]["QSTN_TAG"] = "Q" + tag_num;
					tag_num++;
				}
		...
		tag_questions = function() {
			...
			for(var key in question_list) {
				...
				if(question_list[key]['G_ID'] != null) {
					...
					var qstn_tag_id = question_list[key]['BLY_ID'] +'_'+ question_list[key]['QSTN_NUM'];

					var qstn_tag_html = '<div class="qstn-tag-container" id="'+ 'goal_' + qstn_tag_id +'"><p>' + question_list[key]['QSTN_TAG'] + '</p></div>';

					$("#qstn_tag_" + question_list[key]['G_ID']).append(qstn_tag_html);

					qstn_tag_html = '<div class="qstn-tag-container" id="'+ 'finish_' + qstn_tag_id +'"><p>' + question_list[key]['QSTN_TAG'] + '</p></div>';

					$("#finish_qstn_tag_" + question_list[key]['G_ID']).append(qstn_tag_html);

					qstn_tag_html = '<div class="qstn-tag-container" id="'+ 'lose_' + qstn_tag_id +'"><p>' + question_list[key]['QSTN_TAG'] + '</p></div>';

					$("#lose_qstn_tag_" + question_list[key]['G_ID']).append(qstn_tag_html);
				}
			}
		}
							...
							if(Question.list[key].status == "current question") {
								...
								if(!hasErrors) {
									...
									if(ansCount <= correctAns) {
										...
										$("#goal_" + Question.list[key].id + " p").css("color", "gold");
										$("#finish_" + Question.list[key].id + " p").css("color", "gold");
										$("#lose_" + Question.list[key].id + " p").css("color", "gold");
										...
										$("#goal_" + Question.list[key].id + " p").css("color", "red");
										$("#finish_" + Question.list[key].id + " p").css("color", "red");
										$("#lose_" + Question.list[key].id + " p").css("color", "red");
								...
								if(questionStat.total_questions == (questionStat.correct_ans + questionStat.wrong_ans)) {
									...
									// var qstn_info = Question.getRandomQuestion(bullyId);
									// var question_id = bullyId + "_" + qstn_info.qstn_num;
									
									// player.currentQuestion.questionNum = qstn_info.qstn_num;

									// if(Question.list[question_id] && !Question.list[question_id].isAsked && Question.list[question_id].status == "not answered") {
									...
									// 	self.currentQuestion = {bully: bullyId, questionNum: random_qstn.qstn_num};
		...
		Player = function(id, imgSrc, width, height, x, y, hpMax) {
			...
			self.update = function() {
				...
				if(self.isEnemyInRange()) {
					...
					if(!self.currentQuestion.questionNum && !self.currentQuestion.bully) {

						random_qstn = Question.getRandomQuestion(bullyId);
						console.log(random_qstn);

						if(random_qstn.status) {

							console.log("here");
							
							question_id = bullyId + "_" + random_qstn.qstn_num;
		...
		Question = function(id, qstnType, qstnNum, bullyId, dialog, answer, qstnTag) {
			...
			self.showQuestion = function() {
				...
				$.ajax({
					...
					data: {dialog: self.dialog, tag: self.qstnTag, bully_thumb: Bully.list[self.bully].thumb.src, bully_name: Bully.list[self.bully].name},
					success: function(res) {
				...
				// }).then(function(result) {
					...
					// if(result.status) {
						...
						for(var key in question_list) {
							...
							Question(questionId, question_list[key].QSTN_TYPE, parseInt(question_list[key].QSTN_NUM), question_list[key].BLY_ID, question_list[key].QSTN_DIALOG, answers, question_list[key].QSTN_TAG);
						}
		...
		Question.statusCheck = function(bullyId) {
			...
			for(var key in Question.list) {
				...
				if(Question.list[key].bully == bullyId) {
					...
					if(Question.list[key].status == "current question") {
						curr_question++;
					} else if(Question.list[key].status == "not answered") {
						not_answered++;
					}
			...
			return {total_questions: total_questions, correct_ans: correct_ans, wrong_ans: wrong_ans, curr_question: curr_question, not_answered: not_answered};
		}

		Question.getRandomQuestion = function(blyId) {

			var blyQuestions = [];
			var randomQuestionNum = 0;

			var qstn_status = Question.statusCheck(blyId);
			console.log(qstn_status);

			if(qstn_status.not_answered > 0) {

				for(var key in Question.list) {

					if(Question.list[key].bully == blyId) {

						blyQuestions.push(Question.list[key]);
					}
				}
				
				if(blyQuestions.length > 0) {
					
					do {
						randomQuestionNum = Math.floor((Math.random() * blyQuestions.length) + 1);
					} while(Question.list[blyId + "_" + randomQuestionNum].status != "not answered");
				} else {
					return {status: false, message: "no questions assigned"};
				}
			} else {
				return {status: false, message: "no questions to ask"};
			}

			return {status: true, qstn_num: randomQuestionNum};
		}
